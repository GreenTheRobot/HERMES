...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.preserve_last_img_hidden = False
    ...
    def forward(
        ...
        if image_mask is not None and past_key_values is None:
            ...
            if past_key_values is None:
                if (
                    not getattr(self, "preserve_last_img_hidden", False)
                    or self.last_img_hidden is None
                ):
                    self.last_img_hidden = torch.zeros_like(
                        hidden_states[0, :1, ...]
                    )
                else:
                    self.last_img_hidden = self.last_img_hidden.to(hidden_states)
    ...
    def topK_genrate(
        ...
        embed_weights=None,
        image_mask=None,
        return_layer_timings=False,
        timing_sync_device=None,
    ):
        ...
        depth = self.depth
        top_k = self.top_k
        timing_device = timing_sync_device or hidden_states.device
        layer_timings = []

        def sync_for_timing():
            if (
                torch.cuda.is_available()
                and torch.device(timing_device).type == "cuda"
            ):
                torch.cuda.synchronize(timing_device)

        def start_timer():
            if not return_layer_timings:
                return None
            sync_for_timing()
        ...
        latency = finish_timer(timing_start)
        if return_layer_timings:
            layer_timings.append(
                {
                    "tree_depth": 0,
                    "phase": "draft_initial",
                    "frontier_tokens": 1,
                    "candidate_tokens": int(top_k),
                    "selected_frontier_tokens": int(topk_index.shape[-1]),
                ...
                layer_timings.append(
                    {
                        "tree_depth": int(i + 1),
                        "phase": "draft_expand",
                        "frontier_tokens": int(input_hidden.shape[1]),
                        "candidate_tokens": int(cu_scores.numel()),
                        "selected_frontier_tokens": int(topk_cs_index.numel()),
                        "latency_seconds": latency,
                    }
                )
            scores = topk_cs_p
        ...
        pack_latency = finish_timer(timing_start)
        if return_layer_timings:
            return (
                draft_tokens,
                retrieve_indices,
