...
class LlavaOneVision_SlidingWindow(LlavaOnevisionForConditionalGeneration, Abstract_Hermes):
    """LLaVA-OneVision baseline that retains only the latest visual KV window."""
    ...
    def predict_and_compress(self):
        ...
        for seq_len in current_lengths:
            prefix_len = min(self.visual_start_idx, seq_len)
            recent_start = max(prefix_len, seq_len - self.kv_size)
            keep_indices = torch.cat(
                [
                    torch.arange(prefix_len, device=self.device, dtype=torch.long),
                    torch.arange(recent_start, seq_len, device=self.device, dtype=torch.long),
                ]
            )
            keep_indices_per_layer.append(keep_indices)

        # Reuse the model-specific RoPE correction, but disable HERMES's
        ...
        self._prune_activity_cache(keep_indices_per_layer)
...
def load_model(model_path='llava-onevision-qwen2-7b-ov-hf',
    ...
    model = LlavaOneVision_SlidingWindow.__new__(LlavaOneVision_SlidingWindow)
